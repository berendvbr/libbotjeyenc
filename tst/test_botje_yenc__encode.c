...
void test_botje_yenc__encode_test(void **state) {
    ...
    for(int64_t i = 0; i < 256; i++) {
        ...
        unsigned char test_char = i + 42;
        if(test_char % 256 == 0 || test_char % 256 == 10 || test_char % 256 == 13 || test_char % 256 == 61) {
            assert_int_equal(num_chars, 2);
            unsigned char str_dst_cmp[] = { 61, test_char + 64 };
    ...
    str_src_len = 4;
    memset(str_dst, 0, buf_size);
    unsigned char str_src_2[] = { 'A', 'B', 'C', 'D' };  // 'A' is decimal 65
    num_chars = botje_yenc__encode(str_dst, str_src_2, str_src_len);
    assert_int_equal(num_chars, 4);
    unsigned char str_dst_cmp_2[] = { 'k', 'l', 'm', 'n' };  // 'k' is decimal 107
    ...
    str_src_len = 4;
    memset(str_dst, 0, buf_size);
    unsigned char str_src_3[] = { 19, 214, 224, 227 };  // after +42 special char '=', NUL, LF, CR
    num_chars = botje_yenc__encode(str_dst, str_src_3, str_src_len);
    assert_int_equal(num_chars, 8);
    unsigned char str_dst_cmp_3[] = { '=', '}', '=', '@', '=', 'J', '=', 'M' };
    ...
    str_src_len = 8;
    memset(str_dst, 0, buf_size);
    unsigned char str_src_4[] = { 'A', 'B', 'C', 'D', 19, 214, 224, 227 };
    num_chars = botje_yenc__encode(str_dst, str_src_4, str_src_len);
    assert_int_equal(num_chars, 12);
    unsigned char str_dst_cmp_4[] = { 'k', 'l', 'm', 'n', '=', '}', '=', '@', '=', 'J', '=', 'M' };
